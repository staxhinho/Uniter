...
pub fn volume_logic(input: f64, input_type: &str, output_type: &str) -> f64 {
    ...
    if output_type == "m3" {
        ...
        output = mid_volume * 1000.0;
    } else if output_type == "cm3" {
        output = mid_volume * 1_000_000.0;
    } else if output_type == "mm3" {
        output = mid_volume * 1_000_000_000.0;
    } else if output_type == "hl" {
        output = mid_volume * 10.0;
    } else if output_type == "l" {
        output = mid_volume * 1000.0;
    } else if output_type == "dl" {
        output = mid_volume * 10_000.0;
    } else if output_type == "cl" {
        output = mid_volume * 100_000.0;
    } else if output_type == "ml" {
        output = mid_volume * 1_000_000.0;
    } else if output_type == "ft3" {
        output = mid_volume / 0.0283168;
    } else if output_type == "in3" {
        output = mid_volume / 0.0000163871;
    } else if output_type == "yd3" {
        output = mid_volume / 0.764555;
    } else if output_type == "ac3" {
        output = mid_volume / 1233.48;
    } else if output_type == "gal" {
        output = mid_volume / 0.00378541;
    } else if output_type == "bbl" {
        output = mid_volume / 0.158987;
    }
...
fn dialog() {
    ...
    let output = volume_logic(input, input_type, output_type);
